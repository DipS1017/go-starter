...
type Params struct {
	CurrentPage int32  // Current page number (1-based)
...
func GetPaginationParams(q url.Values, opts ...PaginationOption) *Params {
	...
	params.Offset = calculateOffset(params.CurrentPage, params.Limit)
...
// GetTotalPages calculates the total number of pages based on the total count of items.
// It ensures there is always at least 1 page, even if the count is 0.
func GetTotalPages(count int64, limit int32) int32 {
	// Avoid division by zero
	if limit <= 0 {
		return 1
	}

	// Optional: Return 1 page for empty results (UI preference)
	if count == 0 {
		return 1
	}

	// Ceiling division formula: (x + y - 1) / y
	return int32((count + int64(limit) - 1) / int64(limit))
}
